...
% Title etc
\title[RA2/b Meeting]{RA2/b \\ Status of Lost-Lepton Method \\ \& \ttbar, \wpj Background Rejection Studies }
...
\begin{frame}
  \frametitle{Lost-lepton suppression}
  \begin{figure}
    \centering
  ...
  \item In most bins, main background is top, W+jet events with lost lepton
  \item T1tttt events have leptons $\sim 70 \%$ of the time--but many
    of these are lost as well and end up in our ``hadronic'' search region
  \item Goals   
    \begin{itemize}
    \item Understand lepton composition (e/mu from W,
      hadronic/leptonic tau, dilepton) of lost-lepton BG (ttbar,
      W+jets) and signal (T1tttt)
    \item Test methods for suppressing these backgrounds: isolated track veto, single-prong tau veto.
    \end{itemize}
  \end{itemize}
\end{frame}
...
%  \caption{Awesome figure}
 \end{figure}
      ...
      \item Select a control sample (CS) of exactly one well isolated \\electron, $\mu$ within the acceptance
        ...
        \item Combine independent prediction from single electron and $\mu$ each 1/2 to final prediction of lost-leptons 
        \end{itemize}
      \end{itemize}
\end{frame}
...
\hspace{0.5cm}$m_{T} = \sqrt{2 \cdot p_{T}(\mu)\cdot \met (1 - \cos(\Delta \Phi))}$
  ...
  \begin{columns}
    \begin{column}{0.4\textwidth}
      ...
      \begin{centering}
      \end{centering}
      \item Correction for as a function of lep $p_{T}$ \& activity
      \end{itemize}
      \vspace{0.3cm}
    \end{column}
    ...
    %  \begin{overpic}[width=0.8\textwidth]{figures/lost-lepton/ControlSample__MTW__MCPrMTWDiLepTTbar+MCPrMTWDiLepW__mu_control_sample.pdf}
       \begin{overpic}[width=0.8\textwidth]{figures/control-sample/ControlSample_Combined_searchbins__MTW__MCEx_vs_MuPrMTWDiLep+ElecPrMTWDiLep__Baseline.pdf}
...
\begin{frame}
 \begin{columns}
  ...
  \item Sever problem when deploying method in low statistical regions which are our search regions (hand full of events)
 \end{itemize}
  \end{column}
  \begin{column}{0.6\textwidth}
 ...
 Example: Region with 3 gen but only 1 CS event \\
   \begin{overpic} [width=0.48\textwidth]{figures/deltaRLowStat.pdf}
     \end{overpic}
     \begin{overpic} [width=0.48\textwidth]{figures/deltaRLowStatReco.pdf}
     \end{overpic} \\
    Control sample lepton in high eff. region  $\rightarrow$ low weight $\rightarrow$ underprediction
...
\begin{frame}
 \frametitle{Solving parametization problem (attempt)}
 \begin{itemize}
  \item In 8TeV RA2 used efficiencies binned in search variables following closely search region definition.
  \begin{itemize}
   \item Good closure
   \item Relativley high statistical uncertainties on efficiency maps
   \item Could not be obtained from data only indirect validation via Tag \& Probe method
  \end{itemize}
  \item Ansatz: Lepton \pt together with activity ($A$) around the lepton in a larger fixed cone size
  \begin{itemize}
   ...
   \item Lose some event information but more resistant to low statistics region problem
  \end{itemize}
   \item Muon activity:
  \begin{itemize}
...
%  \begin{block}{}
%  \centering
%  \Large First tests using this new variable for the reconstruction and isoaltion efficiencies
%  \end{block}
\end{frame}
...
%       \begin{overpic}[width=0.57\textwidth]{figures/lost-lepton/Closure__NVtx__MCPrMTWDiLep_vs_MCEx__csa_Baseline.pdf}
  ...
  \item $\HT > 500 \gev$ , $\MHT>200 \gev$, $\NJets\ge 3$ , $\Delta\Phi_{1,2,3}>$0.5, 0.5, 0.3
   \item Reasonable closre (exp $3862.1 \pm 17.9$, pred $3742.9 \pm 27.4$)
  \end{itemize}
...
\begin{frame}
 \frametitle{}
 \begin{itemize}
 \item Classical lost-lepton method status:
 \begin{itemize}
  ...
  \item Studies ongoing of parametization of efficiencies exploring different variables
 \end{itemize}
 \end{itemize}
 \Large If i have time I will include closure plot for all search bins
...
\begin{frame}
  ...
  \begin{itemize}
  \item Baseline selection: $H_{T}^{\mathrm{miss}}>200$ GeV,
  ...
  \item Object definitions:
    ...
    \end{itemize}
  \item All of the definitions can and will be re-optminized for this
    analysis in Run II.
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Lepton compositon at truth level}
  ...
  \end{columns}
  \begin{itemize} 
  \item Try to reject more $t\bar{t}$ BG with
    \begin{itemize}
    \item Single-prong tau veto
    \item Isolated track veto
    \end{itemize}
  \item For each object, try computing $m_T(i,\; E_{T}^{\mathrm{miss}})$--should be cut off at 100 in
    single-$l$ $(W)$ events
...
\begin{frame}
  \frametitle{Using $m_T(l,\; E_{T}^{\mathrm{miss}})$ to save signal}
  \begin{itemize}
  \item We know that events with 1 true lepton and one source of MET
    ($W\rightarrow l\nu$ decay) should obey
  \item If we only reject events with ``lost lepton candidates'' (taus or
    tracks) satisfying the above (or $m_T<100$), can we save any signal?
    How much ttbar would we keep?
...
\begin{frame}
  \frametitle{Lepton event rejection cutflow $(5\mathrm{fb}^{-1})$}
  \begin{columns}[c] % the "c" option specifies center vertical alignment
    \column{.7\textwidth}
    \begin{figure}[h]
      \centering
...
\begin{frame}
  ...
  \item Isolated track veto with $m_T<100$ restriction shows great
    ...
    $S/\sqrt{B}$--will use this for Phys14 on top of standard
    $e/\mu$ veto.
